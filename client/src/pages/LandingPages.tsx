...
import { Loader2, Globe, Sparkles, ExternalLink, Eye } from "lucide-react";
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {p.slug && (
                        <p className="text-xs text-primary mt-1">/p/{p.slug}</p>
                      )}
                    </div>
                    <div className="flex gap-1.5">
                      {p.slug && (
                        <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => window.open(`/p/${p.slug}`, "_blank")}>
